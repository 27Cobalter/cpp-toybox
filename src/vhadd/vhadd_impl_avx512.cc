...
std::span<uint16_t> VHAdd::CalcV_Impl<VMA512>(uint16_t* src, int32_t size, int32_t offset_x,
  ...
  for (int32_t i = offset_x; i < horizontal; i += step) {
    // _mm512_storeu_epi32(vdptr_ + i, _mm512_setzero_si512());
    int32_t* vaptri = vaptr_ + i;
    ...
    __m512i lo      = _mm512_cvtps_epi32(_mm512_mul_ps(vailo, rv));
    __m512i hi      = _mm512_cvtps_epi32(_mm512_mul_ps(vaihi, rv));
...
std::span<uint16_t> VHAdd::CalcH_Impl<VMA512>(uint16_t* src, int32_t size, int32_t offset_x,
  ...
  for (auto j : std::views::iota(offset_y, vertical)) {
    ...
    // __m512i cross = _mm512_shuffle_epi32(acc, _MM_SHUFFLE(1, 0, 3, 2));
    __m512i cross = _mm512_shuffle_epi32(acc, _MM_PERM_BADC);
    acc           = _mm512_add_epi32(acc, cross);
    // cross         = _mm512_shuffle_epi32(acc, _MM_SHUFFLE(2, 3, 0, 1));
    cross         = _mm512_shuffle_epi32(acc, _MM_PERM_CDAB);
    acc           = _mm512_add_epi32(acc, cross);
    int32_t v     =
      reinterpret_cast<int32_t*>(&acc)[0] + reinterpret_cast<int32_t*>(&acc)[4]
...
std::array<std::span<uint16_t>, 2> VHAdd::CalcVH_Impl<VMA512>(uint16_t* src, int32_t size,
  ...
  for (auto j : std::views::iota(offset_y, vertical)) {
    ...
    // __m512i cross = _mm512_shuffle_epi32(acc, _MM_SHUFFLE(1, 0, 3, 2));                    // h
    __m512i cross = _mm512_shuffle_epi32(acc, _MM_PERM_BADC);                    // h
    acc           = _mm512_add_epi32(acc, cross);                                          // h
    // cross         = _mm512_shuffle_epi32(acc, _MM_SHUFFLE(2, 3, 0, 1));                    // h
  ...
  for (int32_t i = offset_x; i < horizontal; i += step) {                         // v
    ...
    __m512i lo      = _mm512_cvtps_epi32(_mm512_mul_ps(vailo, vr_v));             // v
    __m512i hi      = _mm512_cvtps_epi32(_mm512_mul_ps(vaihi, vr_v));             // v
