...
template <auto Value>
struct IsEven {
  static constexpr bool value = (Value % 16 == 0);
};
...
consteval auto create_filtered_list_impl(std::index_sequence<Indices...>) {
  constexpr std::array values = {(Predicate<Min + Indices>::value ? Min + Indices : -1)...};
...
int main() {
  ...
  auto filtered_list = concatenate<s1>(filtered_list0, filtered_list1, filtered_list2,
                                            filtered_list3, filtered_list4, filtered_list5,
                                            filtered_list6, filtered_list7);
